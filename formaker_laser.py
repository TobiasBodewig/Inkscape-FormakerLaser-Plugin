#!/usr/bin/env python 
"""
Modified by Tobias Bodewig 2016, tbodewig@web.de
...
class laser_gcode(inkex.Effect):
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--laser-speed",                     action="store", type="int",             dest="laser_speed",                         default="1000",                          help="Laser speed (mm/min)")
        ...
        self.OptionParser.add_option("",   "--focus-depth",                     action="store", type="string",          dest="focus_depth",                         default="17",                           help="Depth of laser focus")
...
###
################################################################################
    def generate_gcode(self, curve, layer, depth, isFirst):
        tool = self.tools
    ...
    def laser(self) :
        ...
        for layer in self.layers :
            if layer in paths :
                ...
                gcodefirst += self.generate_gcode(curve, layer, 0, isFirst)
                gcode += self.generate_gcode(curve, layer, 0, False)
                isFirst = False
            
        self.export_gcode(gcodefirst,gcode)
